...
func (r *swipePostgresRepo) CreateSwipe(ctx context.Context, s *domain.Swipe) error {
	...
	err := r.db.QueryRow(ctx, query, s.UserID, s.TargetUserID, s.SwipeType, s.CreatedAt, s.UpdatedAt).Scan(&s.ID, &s.SwipeDate)
	if err != nil {
...
func (r *swipePostgresRepo) HasSwiped(ctx context.Context, userID, targetUserID uint) (bool, error) {
	...
	`
	var exists bool
	err := r.db.QueryRow(ctx, query, userID, targetUserID).Scan(&exists)
	if err != nil {
...
func (r *swipePostgresRepo) CreateMatch(ctx context.Context, user1ID, user2ID uint) (*domain.Match, error) {
	// Canonical ordering so user1_id < user2_id to prevent duplicates
	if user1ID > user2ID {
	...
	`
	var m domain.Match
	err := r.db.QueryRow(ctx, query, user1ID, user2ID, time.Now()).Scan(&m.ID, &m.User1ID, &m.User2ID, &m.MatchedAt)
	if err != nil {
